...
is_port_free() {
    local port=$1
    # Проверяем, слушает ли кто-то порт (TCP или UDP)
...
security_hardening() {
    # --- 2.1 СМЕНА ПОРТА SSH (ИСПРАВЛЕННАЯ ЛОГИКА) ---
    if ask "Изменить порт SSH с 22 на $DEFAULT_SSH_PORT?"; then
        # ПРОВЕРКА ВЫПОЛНЯЕТСЯ ЗДЕСЬ, ДО ИЗМЕНЕНИЯ КОНФИГА
        if is_port_free "$DEFAULT_SSH_PORT"; then
            ...
            systemctl restart ssh || { error "Ошибка перезапуска SSH (возможно, ошибка в конфиге)"; exit 1; }
            success "Порт SSH успешно изменен на $DEFAULT_SSH_PORT."
        else
            error "Порт $DEFAULT_SSH_PORT занят. Изменение порта SSH отменено."
            warn "Пожалуйста, освободите порт $DEFAULT_SSH_PORT или перезапустите скрипт позже."
            # Скрипт НЕ будет менять порт, если он занят.
        fi
    fi
    ...
    if grep -q "PermitRootLogin yes" /etc/ssh/sshd_config; then
        ...
        systemctl restart ssh || true # Игнорируем ошибку, если SSH уже упал из-за неверного конфига выше
    ...
    if ask "Установить и настроить UFW (рекомендуется)?"; then
        ...
        # Разрешаем наш SSH порт (если он был успешно изменен или стандартный)
    ...
    # --- 2.5 Отключение IPv6 (АВТОМАТИЧЕСКИ) ---
    info "🌐 Применяем настройки для отключения IPv6..."
...
EOL
    sysctl -p
    success "IPv6 отключен (автоматически)."
}
...
install_telemt() {
    ...
    while true; do
        ...
        if is_port_free "$PORT"; then
            ...
            # Покажем, кто занял порт, чтобы было понятнее
    ...
    # Получаем СЛУЧАЙНЫЙ домен для TLS маскировки. Если не удалось, используем duckduckgo.com.
    DOMAIN=$(curl -s https://raw.githubusercontent.com/nolaxe/install-MTProxy/main/site.txt | shuf -n 1 2>/dev/null || echo "duckduckgo.com")
    
    MAIN_SECRET=$(openssl rand -hex 16)
    
    # Используем защищенный HereDoc, чтобы избежать ошибок с кавычками и переменными.
    cat > telemt.toml << 'EOF'
show_link = ["docker"]
[general]
fast_mode = true
